...
contract PosiNFTFactory is Governance, IPosiNFTFactory {
    ...
    event GegoBurn(uint256 indexed id, uint256 amount, address erc20);
    ...
    {
        require(
            _ruleProxys[nftType] == IGegoRuleProxy(address(0)),
            "must null"
        );
    ...
    function setBaseResId(uint256 tokenId, uint256 resBaseId)
        external
        onlyGovernance
    {
        require(_gegoes[tokenId].id > 0, "gego not exist");
    ...
    function mint(MintData memory mintData) public lock {
        ...
        require(
            _ruleProxys[mintData.nftType] != IGegoRuleProxy(address(0)),
            " invalid mint nftType !"
        );
        ...
        _randomNonce++;
        IPosiNFT.Gego memory gego = _ruleProxys[mintData.nftType].generate(
            msg.sender,
            mintData.ruleId,
            _randomNonce
        );
        ...
        gego.createdTime = gego.createdTime > 0
            ? gego.createdTime
            : block.timestamp;

        gego.amount = gego.amount > 0 ? gego.amount : mintAmount;
        gego.resBaseId = gego.resBaseId > 0
            ? gego.resBaseId
            : mintData.resBaseId;
    ...
    function gmMint(MintData memory mintData, MintExtraData memory extraData)
        public
    {
